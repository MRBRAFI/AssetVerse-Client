...
import { FiSearch, FiPackage, FiCalendar, FiBriefcase, FiArrowRight, FiFilter, FiDownload } from "react-icons/fi";
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
          <div className="space-y-2">
            ...
            >
              ...
              <span className="text-xs font-black uppercase tracking-[0.3em]">Personal Inventory</span>
            </motion.div>
            <motion.h1 
            ...
            >
              My <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-indigo-600">Assets</span>
            </motion.h1>
          ...
          >
             <div className="px-6 py-2 border-r border-gray-100">
                ...
                <p className="text-2xl font-black text-blue-600 leading-none">{assets.length}</p>
             </div>
          ...
          <AnimatePresence mode="popLayout">
            {filteredAssets.length === 0 ? (
              ...
              >
                ...
                <h3 className="text-2xl font-black text-gray-900 uppercase tracking-tighter mb-2">No Assets Linked</h3>
                <p className="text-gray-400 font-bold uppercase text-xs tracking-widest">Ensure your requests have been synchronized by HR</p>
              </motion.div>
            ) : (
                   ...
                   <div className="mb-6 relative z-10">
                      ...
                      <div className="space-y-3">
                         <div className="flex items-center gap-3 text-gray-400 group-hover:text-gray-600 transition-colors">
                            ...
                            <span className="text-[10px] font-black uppercase tracking-widest">{asset.companyName || "N/A Corporation"}</span>
                         </div>
                         <div className="flex items-center gap-3 text-gray-400 group-hover:text-gray-600 transition-colors">
                            <FiCalendar size={14} className="text-blue-500/50" />
                            <span className="text-[10px] font-black uppercase tracking-widest">{formatDate(asset.assignmentDate)}</span>
                         </div>
                      </div>
                   </div>
                   ...
                   <div className="flex items-center justify-between pt-6 border-t border-gray-50 relative z-10">
                      <div className="flex items-center gap-2">
                        <div className={`w-2 h-2 rounded-full animate-pulse ${
                          asset.status === 'assigned' ? 'bg-green-500' : 'bg-amber-500'
                        }`} />
                        <span className="text-[10px] font-black text-gray-900 uppercase tracking-widest">{asset.status}</span>
                      </div>
                      <motion.button 
                        whileHover={{ x: 5 }}
